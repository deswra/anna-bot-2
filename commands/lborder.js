...
async function getBorder() {
  ...
  let count = 0;
  const eventPtsSummaryRes = await fetch(`https://api.matsurihi.me/mltd/v1/events/${currentEvent.id}/rankings/summaries/loungePoint?prettyPrint=false`);
  ...
  const bordersSummaryRes = await fetch(`https://api.matsurihi.me/mltd/v1/events/${currentEvent.id}/rankings/logs/loungePoint/10,50,100,250,500?prettyPrint=false`);
